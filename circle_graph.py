		...
		while  abs(sum_der) > eps: 
			...
			for Garden_plant in self.plantsList:
				...
				for friend in Garden_plant.friendPlants:
					...
					Gr   = Garden_plant.ray
					Gf   = friend.ray
					if distance_plants(Garden_plant,friend)<friend.safety_ray+Garden_plant.safety_ray and dist >= Gr-Gf and dist > Gf-Gr:
						update_center   += -0.1*derivate_spring(Garden_plant,friend)+derivate_diff_surf(Garden_plant,friend,"safety")-1*derivate_diff_surf(Garden_plant,friend)
						sum_der         += np.sum(update_center)
					elif distance_plants(Garden_plant,friend)<friend.ray+Garden_plant.ray and dist >= Gr-Gf and dist > Gf-Gr :
...
if __name__ == '__main__':
	...
	t = time.time()
	g.optimize_garden(eps=1e-6,LR=1e-4)
	print(time.time()-t)
	print(P2.center,P2.name,P2.color)
	print(P1.center,P1.name,P1.color)
	plt.show()
